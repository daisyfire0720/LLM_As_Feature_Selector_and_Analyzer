...
class SelectionMechanismAnalyzer:
    ...
    # helper for optional progress bars
    def _iter(self, iterable, desc=None, leave=True, total=None, position=None, **tqdm_kwargs):
        """
    ...
    # helper for parallel fold processing in CV routines
        ...
        for pred_obj in self._iter(self.pred_objs, desc="Targets (sensitivity)"):
            ...
            for pv in self._iter(perturb_values, desc=f"Perturbations ({mode}) for {pred_obj}", leave=False):
                for method, add_list in self._iter(add_dict.items(), desc="Methods", leave=False):
                    ...
                    def process_perturb_fold(fold, tr, te):
    ...
    def plot_sensitivity_mean_curves(self, sens_summary_df: pd.DataFrame, metric: str = "rmse", save: bool = True, max_panels: int = 6):
        """
