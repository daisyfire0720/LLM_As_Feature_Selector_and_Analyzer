...
class LLMConfig:
    ...
    temperature: float = float(os.getenv("GEMINI_TEMPERATURE", "0.05"))
...
class FeatSelector:
    ...
    def _build_prompt_feature_selector(self, scen_context: str, feature_table: str, with_domain: bool, with_value: bool) -> str:
        if with_domain is True and with_value is True:
            prompt_file = os.path.join(self.feat_path, 'single_feat_prompt', 'featselect.txt')
        if with_domain is False and with_value is True:
            prompt_file = os.path.join(self.feat_path, 'single_feat_prompt', 'featselect_wo_scen.txt')
        if with_domain is True and with_value is False:
            prompt_file = os.path.join(self.feat_path, 'single_feat_prompt', 'featselect_wo_val.txt')
        if with_domain is False and with_value is False:
            prompt_file = os.path.join(self.feat_path, 'single_feat_prompt', 'featselect_wo_scen_val.txt')
    ...
    def llm_feature_selector(self, df_merged: pd.DataFrame, with_domain: bool = True, with_value: bool = True) -> Dict[str, Any]:
...
if __name__ == '__main__':
    ...
    feat_path = ''
    output_path = '../output_single_featimp/'
    top_n = 50
    top_k = 25
    final_n_lst = [20, 25]
    pred_obj_lst = ['step_x']
...
#%%% loop for use
# if __name__ == '__main__':
#     os.environ["GEMINI_API_KEY"] = "AIzaSyBYkVqy_Fh8BtytlMtJXeM8nWIgpskJsKU"
...
#     top_n = 50
#     top_k = 25
#     feat_dict_10 = {'final_n': 10, 'n_core': 5, 'n_rare': 3, 'n_art': 2}
#     feat_dict_15 = {'final_n': 15, 'n_core': 7, 'n_rare': 5, 'n_art': 3}
#     feat_dict_20 = {'final_n': 20, 'n_core': 10, 'n_rare': 6, 'n_art': 4}
#     feat_dict_25 = {'final_n': 25, 'n_core': 12, 'n_rare': 8, 'n_art': 5}
#     feat_dict_lst = [feat_dict_10, feat_dict_15, feat_dict_20, feat_dict_25]
#     pred_obj_lst = ['step_x', 'step_y']
#     flow_type_lst = ['corner', 'intersection', 'entrance']
#     for pred_obj in pred_obj_lst:
#         for flow_type in flow_type_lst:
#             for feat_dict in feat_dict_lst:
#                 try:
#                     # Your code here
#                     explainer = FeatSelector(input_path, feat_path, output_path, flow_type, pred_obj, top_n, top_k, feat_dict)
#                     candidate_dict = explainer.all_feature_selector(save = True)
#                     print("Sucssessfully generated candidate features for", flow_type, pred_obj, feat_dict['final_n'])
#                     # rest 2 minute
#                     time.sleep(120)
#                 except KeyError as e:
#                     traceback.print_exc()
